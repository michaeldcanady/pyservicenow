...
def test_attachment_url():
    ...
    hardware_request = client.Now.table_api(table_name).request.sysparam_limit(1).Get
...
def test_get_first_attachment():
    ...
    hardware = client.Now.table_api("alm_hardware").request.sysparam_limit(1).Get.invoke_request
